...
class MockAudioWorkletNode {
  port = { postMessage: vi.fn(), onmessage: null as ((event: { data: unknown }) => void) | null };
  connect = vi.fn();
  disconnect = vi.fn();
...
class MockAudioContext {
  ...
  createBuffer = vi.fn().mockReturnValue({ getChannelData: () => new Float32Array(1600) });
...
describe("LocalWebSocketAdapter Specifications", () => {
  ...
  it("sätter status till 'error' om WebSocket kastar fel", async () => {
    ...
    class ErrorWebSocket extends MockWebSocket {
      constructor(url: string) {
        super(url);
        setTimeout(() => { this.onerror?.(new Error("Network unreach")); }, 5);
      }
    }
