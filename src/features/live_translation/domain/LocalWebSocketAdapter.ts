...
export class LocalWebSocketAdapter implements AudioTransportAdapter {
  ...
  public getStatus(): AudioTransportStatus { return this.status; }
  public getRemoteStream(): MediaStream | null { return this.remoteStream; }
  public getPublishedTrack(): string | null { return this.publishedTrack; }
  ...
  private setStatus(newStatus: AudioTransportStatus): void {
    if (this.status !== newStatus) {
      ...
      for (const listener of this.statusListeners) listener(newStatus);
    }
  }
  ...
  public disconnect(): void {
    ...
    if (this.sourceNode) {
      try { this.sourceNode.disconnect(); } catch (e) { console.debug("[LocalWS] source disconnect:", e); }
      this.sourceNode = null;
    }
    if (this.micWorkletNode) {
      try { this.micWorkletNode.disconnect(); } catch (e) { console.debug("[LocalWS] micWorklet disconnect:", e); }
      this.micWorkletNode = null;
    }
    if (this.processorWorkletNode) {
      try { this.processorWorkletNode.disconnect(); } catch (e) { console.debug("[LocalWS] processor disconnect:", e); }
  ...
  public async publishAudio(track: MediaStreamTrack): Promise<string | null> {
    ...
    try {
      ...
      if (ctx.audioWorklet?.addModule) {
        ...
        } catch (e) { console.debug("[LocalWS] MicCapture fallback:", e); }
      }
      ...
      if (typeof AudioWorkletNode !== "undefined") {
        ...
        workletNode.port.onmessage = (event: { data: ArrayBuffer }) => { this.sendAudioFrame(event.data); };
  ...
  public sendAudioFrame(data: ArrayBuffer | Int16Array): boolean {
    ...
    this.ws.send(data instanceof ArrayBuffer ? data : data.buffer);
    return true;
  }
  ...
  public async subscribeToTrack(remoteSessionId: string, trackName: string): Promise<void> {
    ...
    if (ctx.audioWorklet?.addModule) {
      ...
      } catch (e) { console.debug("[LocalWS] AudioProcessor fallback:", e); }
    }
    ...
    const sabAudio = hasSAB ? new SharedArrayBuffer(BUFFER_SIZE * 4) : new ArrayBuffer(BUFFER_SIZE * 4);
    const sabPointers = hasSAB ? new SharedArrayBuffer(8) : new ArrayBuffer(8);
    ...
    if (typeof AudioWorkletNode !== "undefined") {
      ...
      if (this.destinationNode) processorNode.connect(this.destinationNode);
      if (ctx.destination) {
        try { processorNode.connect(ctx.destination); } catch (e) { console.debug("[LocalWS] dest connect:", e); }
      }
    }
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      this.ws.send(JSON.stringify({ type: "subscribe", remoteSessionId, trackName }));
    }
  }
  ...
  private handleIncomingMessage(event: { data: ArrayBuffer | string }): void {
    ...
    if (event.data instanceof ArrayBuffer && this.processorWorkletNode) {
      this.processorWorkletNode.port.postMessage({ type: "PCM_DATA", payload: event.data });
    }
  }
}
